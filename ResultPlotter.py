...
class ResultPlotter:
    ...
    def load_sca_model(self, model_file):
    ...
    def _rank(self, predictions, metadata, real_key, min_trace_idx, max_trace_idx, last_key_bytes_proba):
        # Compute the rank
    ...
    def _full_ranks(self, model, dataset, metadata, min_trace_idx, max_trace_idx, rank_step):
        ...
        if max_trace_idx > dataset.shape[0]:
            print(
                "Error: asked trace index %d overflows the total traces number %d" % (max_trace_idx, dataset.shape[0]))
            sys.exit(-1)
        ...
        f_ranks = np.zeros((len(index), 2), dtype=np.uint32)  # 0
        key_bytes_proba = []
        for t, i in zip(index, range(0, len(index))):
            real_key_rank, key_bytes_proba = self._rank(predictions[t - rank_step:t], metadata, real_key, t - rank_step,
                                                        t, key_bytes_proba)
    ...
    # database
    def _load_ascad(self, ascad_database_file, load_metadata=False):
        self._check_file_exists(ascad_database_file)
        ...
        X_attack = scaler_X.fit_transform(X_attack)
    ...
    def check_model(self, model_file, ascad_database, num_traces=2000):  # 2000
        ...
        (X_profiling, Y_profiling), (X_attack, Y_attack), (Metadata_profiling, Metadata_attack) = \
            self._load_ascad(ascad_database, load_metadata=True)
        # Load model
        model = self.load_sca_model(model_file)
        # We test the rank over traces of the Attack dataset, with a step of 10 traces
        ranks = self._full_ranks(model, X_attack, Metadata_attack, 0, num_traces, 10)
        # We plot the results
